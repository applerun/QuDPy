...
4. Probe-only is still run only once and reused for all delays.

5. All heavy numerical operations still go through existing QuDPy APIs.
"""
...
class TaResultIO:
    ...
    def _plot_time_traces_for_delay(self, result: TaResult, item: TaDelayResult, trace_dir: Path) -> Path:
        ...
        ax.plot(t_fs, _norm(E_total), linewidth=1.0, label="E_total(t), normalized")
        ax.plot(t_fs, _norm(np.real(P_pp)), linewidth=1.0, label="Re P_pump_probe(t), normalized")
        ax.plot(t_fs, _norm(np.real(P_probe)), linewidth=1.0, label="Re P_probe_only(t), normalized")
        ax.plot(t_fs, _norm(delta), linewidth=1.0, label="Re ΔP(t), normalized")

        ax.set_title(f"Time traces of delay time {_pretty_delay_text(item.delay_fs)} fs")
        ...
        # User requested this naming pattern and folder.
