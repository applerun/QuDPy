...
`meta.json` 包含 mode、physical params、solver params、time scale、input drive 的 `to_dict()` / `to_expr()`、trace error 和 Hermiticity error。`components.csv` 对 two-level result 输出 `time_fs`、`rho11`、`rho22`、`Re_rho12`、`Im_rho12`、`abs_rho12`，如果 result 有 drive 还会包含 `drive_value`。
