...
from sjh_learn.utils.spectroscopy.absorption_spectra import lab_frame_fft_response_legacy as lab_frame_fft_response
...
def as_dynamics_result(result) -> DynamicsResult:
	"""Return a DynamicsResult view for analysis/plotting.

	New run_case returns PieceDynamicsResultSeries. Legacy analysis and
	spectroscopy code still operate on a single DynamicsResult, so use
	series.stitch() explicitly at this boundary.
	"""

	if isinstance(result, DynamicsResult):
		return result

	# Avoid triggering PieceDynamicsResultSeries.__getattr__ during probing.
	stitch_method = getattr(type(result), "stitch", None)
	if callable(stitch_method):
		stitched = stitch_method(result)
	...
	raise TypeError(
		"run_case result must be DynamicsResult or expose stitch() -> DynamicsResult; "
...
def result_ckp_path(output_dir: Path, case_name: str) -> Path:
	...
	return output_dir / "checkpoints" / f"{case_name}_piecewise_series_v2.ckp"
...
def run_case_and_extract(
	...
	result_series = run_case(
		params,
		load_ckp = ckp_path,
		save_ckp = ckp_path,
		force_run = FORCE_RERUN,
	)
	result = as_dynamics_result(result_series)
