...
def lab_frame_absorption_response(
	...
	该函数只计算当前主线使用的吸收谱定义
	``absorption = omega * Im[P(omega) / E(omega)]``，不再计算 rho 通道 FFT
	或其它 legacy debug 字段。需要旧 benchmark 输出时再使用
	...
	# 明确保留主定义的可读别名；不要再生成 neg_omega_im_P_over_E。
	response["omega_im_P_over_E"] = response["absorption"]
...
def lab_frame_fft_response_legacy(
	...
	"""Legacy lab-frame pulse FFT diagnostic helper.

	该函数保留旧 benchmark / debug 输出，包括 rho 通道 FFT、P/E、rho/E、
	``neg_omega_im_P_over_E`` 等字段。新吸收谱或 TA producer 主线应优先使用
	``lab_frame_absorption_response``，它只计算 ``absorption = omega * Im[P/E]``，
	避免不必要的 FFT 和中间输出。
	"""

	if warn_legacy:
		warnings.warn(
			"lab_frame_fft_response is a legacy broad diagnostic helper. "
			"Prefer lab_frame_absorption_response for absorption/TA producer workflows; "
			"it computes only absorption = omega * Im(P/E) and avoids extra FFT outputs.",
			UserWarning,
			stacklevel = 2,
		)
