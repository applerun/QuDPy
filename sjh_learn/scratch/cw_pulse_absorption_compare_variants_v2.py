...
def main():
    ...
    # Panel 1: all time-domain pulses together.
    baseline_record = records[0]
    t_fs = baseline_record["time_fs"]
    max_sigma = max(record["variant"].pulse_sigma_fs for record in records)
    time_window = (t_fs >= -8.0 * max_sigma) & (t_fs <= 8.0 * max_sigma)

    with plt.rc_context(custom_plot_style):
        fig, axes = plt.subplots(6, 1, figsize=(15.0, 32.0), sharex=False)

        # (1) Time-domain pulse panel: show all pulses together.
        for record in records:
            variant = record["variant"]
            axes[0].plot(
                t_fs[time_window],
                normalize_for_shape(record["E_t"][time_window]),
                label=variant.label,
                linewidth=2.5,
            )
        axes[0].set_title("Time domain pulse")
        axes[0].set_xlabel("Time (fs)")
        axes[0].set_ylabel("Normalized E(t)")
        axes[0].legend(loc="best")

        # (2)-(6) One spectral panel per condition.
        for ax, record in zip(axes[1:], records):
            variant = record["variant"]
            response = record["response"]
            band = (response["energy_eV"] >= e_min) & (response["energy_eV"] <= e_max)
            x = response["energy_eV"][band]
