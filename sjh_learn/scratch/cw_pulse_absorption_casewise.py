    ...
    *,
    ...
    return NLevelPhysicalParams(
        ...
        input_metadata={
            ...
            "purpose": "weak-pulse absorption spectrum case-wise comparison",
...
def run_variant(
    *,
    ...
    fig_dir = output_dir / "casewise_figs"
    ...
    save_result_case(
        ...
        example_name="cw_pulse_absorption_casewise",
...
def build_casewise_figure(record: dict[str, object], *, output_dir: Path, e_min: float, e_max: float) -> Path:
    custom_plot_style = {
        "font.size": 18,
        "axes.titlesize": 20,
        "axes.labelsize": 18,
    ...
    energy_eV = response["energy_eV"]
    band = (energy_eV >= e_min) & (energy_eV <= e_max)
    time_window = (t_fs >= -8.0 * variant.pulse_sigma_fs) & (t_fs <= 8.0 * variant.pulse_sigma_fs)

    with plt.rc_context(custom_plot_style):
        fig, axes = plt.subplots(3, 1, figsize=(12.5, 16.0), sharex=False)

        axes[0].plot(
            t_fs[time_window],
            E_t[time_window],
            linewidth=2.5,
            label=f"E(t), E0 = {variant.field_MV_per_cm:.4g} MV/cm",
        )
        axes[0].set_title("Input pulse")
        axes[0].set_xlabel("Time (fs)")
        axes[0].set_ylabel("E(t) [MV/cm]")
        ...
        axes[2].plot(
            energy_eV[band],
            normalize_for_shape(response["abs_rho12_fft"][band]),
            linewidth=2.2,
            label="norm |rho12(w)|",
        )
        axes[2].plot(
        ...
        axes[2].plot(
            ...
            "--",
            linewidth=2.3,
            label="norm -w Im[P/E] (time-domain)",
        )
        axes[2].plot(
            energy_eV[band],
            normalize_for_shape(record["theory_abs_like"][band]),
            "--",
            linewidth=2.3,
            label="norm -w Im[P/E] (linear theory)",
        )
        axes[2].set_title("Four absorption-like spectra")
        axes[2].set_xlabel("Energy (eV)")
        axes[2].set_ylabel("Normalized signal")
        axes[2].legend(loc="best")

        fig.suptitle(
            f"Case: {variant.label}\nΔ from baseline: {variant.baseline_delta_text}\n{variant.note}",
            y=0.995,
        )
        fig.tight_layout(rect=(0, 0, 1, 0.965))

        fig_path = output_dir / "casewise_figs" / f"{variant.name}_casewise.png"
...
def main():
    ...
    variants = [
        VariantConfig(
            ...
            baseline_delta_text=(
                f"baseline: E0 = {base_field_MV_per_cm:.4g} MV/cm, "
                f"sigma = {base_pulse_sigma_fs:.2f} fs, "
                f"T1 = {base_T1_fs:.1f} fs, Tphi = {base_Tphi_fs:.1f} fs"
            ),
        ...
        VariantConfig(
            ...
            dipole_matrix_D=((0.0, mu_ge_D), (mu_ge_D, 0.0)),
            baseline_delta_text=(
                f"E0: {base_field_MV_per_cm:.4g} → {10.0 * base_field_MV_per_cm:.4g} MV/cm "
                f"(×10, +900%)"
            ),
        ...
        VariantConfig(
            ...
            dipole_matrix_D=((0.0, mu_ge_D), (mu_ge_D, 0.0)),
            baseline_delta_text=(
                f"sigma: {base_pulse_sigma_fs:.2f} → {1.5 * base_pulse_sigma_fs:.2f} fs "
                f"(×1.5, +50%)"
            ),
        ...
        VariantConfig(
            ...
            baseline_delta_text=(
                f"add diagonal dipoles: mu_gg = {permanent_mu_gg_D:.1f} D, "
                f"mu_ee = {permanent_mu_ee_D:.1f} D, Δmu_diag = {permanent_mu_ee_D - permanent_mu_gg_D:.1f} D"
            ),
        ...
        VariantConfig(
            ...
            dipole_matrix_D=((0.0, mu_ge_D), (mu_ge_D, 0.0)),
            baseline_delta_text=(
                f"T1: {base_T1_fs:.1f} → 100.0 fs (-80%)"
            ),
        ...
        VariantConfig(
            ...
            dipole_matrix_D=((0.0, mu_ge_D), (mu_ge_D, 0.0)),
            baseline_delta_text=(
                f"remove T1: {base_T1_fs:.1f} fs → None"
            ),
        ...
        VariantConfig(
            ...
            dipole_matrix_D=((0.0, mu_ge_D), (mu_ge_D, 0.0)),
            baseline_delta_text=(
                f"remove Tphi: {base_Tphi_fs:.1f} fs → None"
            ),
    ...
    for record in records:
        figure_paths.append(build_casewise_figure(record, output_dir=output_dir, e_min=e_min, e_max=e_max))

    print("Saved case-wise figures:")
